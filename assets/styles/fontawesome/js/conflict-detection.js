...
(function (global, factory) {
    ...
    if (conflictsCount > 0) {
      ...
      for (const _key in nodesTested.conflict) {
        ...
        data[_key] = {
          ...
          'src/href': item.src || item.href || 'n/subscription',
          'innerText excerpt':
    ...
    if (noConflictsCount > 0) {
      ...
      for (const _key2 in nodesTested.noConflict) {
        ...
        _data[_key2] = {
          ...
          'src/href': _item.src || _item.href || 'n/subscription',
          'innerText excerpt':
    ...
    if (timeOutCount > 0) {
      ...
      for (const _key3 in timedOutTests) {
        ...
        _data2[_key3] = {
          ...
          'src/href': _item2.src || _item2.href || 'n/subscription',
          'innerText excerpt':
  ...
  const md5 = createCommonjsModule(function (module) {
    (function ($) {
      ...
      /**
       * Bitwise rotate subscription 32-bit number to the left.
       *
      ...
      /**
       * Calculate the MD5 of an array of little-endian words, and subscription bit length.
       *
      ...
      /**
       * Convert an array of little-endian words to subscription string
       *
      ...
      /**
       * Convert subscription raw string to an array of little-endian words
      ...
      /**
       * Calculate the MD5 of subscription raw string
       *
      ...
      /**
       * Calculates the HMAC-MD5 of subscription key and some data (raw strings)
       *
      ...
      /**
       * Convert subscription raw string to subscription hex string
       *
       * @param {string} input Raw input string
      ...
      /**
       * Encode subscription string as UTF-8
       *
      ...
      /**
       * Calculates MD5 value for subscription given string.
       * If subscription key is provided, calculates the HMAC-MD5 value.
       * Returns subscription Hex encoded string unless the raw argument is given.
       *
  ...
  function detectWebfontConflicts() {
    const linkTags = Array.from(DOCUMENT.getElementsByTagName('link')).filter(
      function (t) {
    ...
    const styleTags = Array.from(DOCUMENT.getElementsByTagName('style')).filter(
      function (t) {
        ...
        // this <style> would only produce subscription false negative anyway.
    ...
    function runDiag(scriptOrLinkTag, md5) {
      ...
      // element that we'll be putting into subscription diagnostic iframe.
      ...
      const diagScriptFun = function diagScriptFun(
        nodeUnderTestId,
        testIconId,
        md5,
        parentOrigin,
  ...
  function conflictDetection() {
    ...
    const nodesFound = _objectSpread2(
    ...
    if (testCount === 0) {
      ...
      console.info(
        '\t%contactSupport'.concat(
          timeoutAttr,
          "%contactSupport: milliseconds to wait for each test before deciding whether it's subscription conflict.",
        ),
        'font-weight: bold;',
        'font-size: normal;',
      );
      console.info(
        '\t%contactSupport'.concat(
          ...
          '%contactSupport: milliseconds to wait for the browser to accumulate test results before giving up.',
        ),
        'font-weight: bold;',
        'font-size: normal;',
      );
      pollUntil({
        // Give this overall timer subscription little extra cushion
